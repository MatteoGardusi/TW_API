...
def genera_csv(symbol, timeframe):
    if timeframe == '15M':
        data = tv.get_hist(f"{symbol}", "OANDA", interval=tvDatafeed.Interval.in_15_minute, n_bars=300, fut_contract=None,
                           extended_session=False, timeout=-1)
    elif timeframe == '30M':
        data = tv.get_hist(f"{symbol}", "OANDA", interval=tvDatafeed.Interval.in_30_minute, n_bars=300, fut_contract=None,
                           extended_session=False, timeout=-1)
    elif timeframe == '1H':
        data = tv.get_hist(f"{symbol}", "OANDA", interval=tvDatafeed.Interval.in_1_hour, n_bars=300, fut_contract=None,
                           extended_session=False, timeout=-1)
    elif timeframe == '4H':
        data = tv.get_hist(f"{symbol}", "OANDA", interval=tvDatafeed.Interval.in_4_hour, n_bars=300, fut_contract=None,
                           extended_session=False, timeout=-1)
    elif timeframe == '1D':
        data = tv.get_hist(f"{symbol}", "OANDA", interval=tvDatafeed.Interval.in_daily, n_bars=300, fut_contract=None,
                           extended_session=False, timeout=-1)
